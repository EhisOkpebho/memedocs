...
export default function Home() {
    ...
    return (
        <div className='mx-auto w-full lg:w-3/4 xl:w-2/3 h-full flex flex-col gap-4 md:gap-8'>
                ...
                <div className='w-full flex flex-col md:flex-row gap-x-4 gap-y-3'>
                    {
                        PROFILES.map(({name, icon: Icon, color}, index) => (
                            <button
                                ...
                                <p className='tracking-wide uppercase font-bold text-lg md:text-sm xl:text-lg'>Je suis {name}</p>
                                <CircleArrowRight className='size-8 group-hover:rotate-90 max-md:rotate-90 duration-300'/>
                            </button>
                        ))
                    }
                </div>
                                ...
                                <div className='px-8 py-8 w-full h-full absolute left-0 top-0 backdrop-blur-sm'>
                                    <div className='px-4 w-full'>
                                        <div className='flex flex-col items-center justify-center md:gap-y-1'>
                                            <small className='uppercase font-bold text-lg md:text-2xl'>Je suis</small>
                                            <span className='uppercase font-extrabold text-2xl md:text-4xl'>{name}</span>
                                        </div>
            ...
            <footer
                className='px-4 md:px-8 py-4 w-full min-h-20 flex flex-col md:flex-row items-center justify-between rounded-2xl shadow-sm bg-white'>
                <p>© 2024 tous droits réservés - Équipe 7</p>
                <p>EPSI - Lille</p>
            </footer>
        </div>
    )
